...
function syncActions(kind = 'standard') {
  ...
  return `<div class="actions sync-actions"><button class="primary" data-action="sync-current-plus-last-days">${benchmarkLabel}</button><button class="secondary" data-action="sync-all">Sync selected day only</button><button class="ghost" data-action="refresh">Refresh</button></div><p class="small">“Sync current + last days” fetches the selected date, WTD previous days, same day last week, and 4-week comparison dates for all POS stores. “Sync selected day only” fetches only the visible reporting date.</p>`;
}
...
function renderToday() {
  ...
  return `<section class="card">
    <div class="pills"><span class="pill">Beta 0.2.31</span><span class="pill">${esc(state.role)}</span><span class="pill">POS sync-first data layer</span></div>
...
function renderSales() {
  return `<section class="card"><h2>Live Sales / Ops</h2>${dateControls()}<p>POS daily/hourly/product data is sync-first from reporting.site. Uploaded POS files are backup only. Uber, Frieda’s/Square and production still use uploaded files/API data.</p>${syncActions('sales')}<div class="actions"><button class="secondary" data-action="sync-uber">Sync Uber</button><button class="secondary" data-action="sync-square">Sync Square</button></div></section>${weeklySummaryCard()}${storeSalesCards()}${staleWarnings()}`;
}
...
function renderImports() {
  ...
  return `<section class="card"><h2>File Imports</h2>${dateControls()}<p>0.2.31 uses browser POS sync for live POS; uploaded files remain the fallback for production, Uber and Frieda/Square. POS Excel/CSV files are backup only; POS daily/hourly/product data should come from Live POS sync.</p><div class="actions"><button class="primary" data-action="reload-imports">Reload uploaded files</button><button class="secondary" data-action="sync-current-plus-last-days">Sync current + last days</button><button class="secondary" data-action="sync-all">Sync selected day only</button><button class="ghost" data-action="diagnostics">Refresh diagnostics</button></div></section><section class="card"><h2>Import folders</h2><table class="table"><tbody>${['pos/product','pos/history','pos/hourly','uber','friedas','production'].map(x => `<tr><td><code>server/data/imports/${x}</code></td><td>${esc(folderMeaning(x))}</td></tr>`).join('')}</tbody></table></section><section class="card"><h2>Import status</h2>${importStatusTable(imports, diag)}</section><section class="card"><h2>Loaded files</h2>${filesTable(imports.files || diag.files || [])}</section>`;
}
function folderMeaning(x) {
  return { 'pos/product':'Optional POS product backup only. reporting.site sync is the primary product source.', 'pos/history':'Optional POS ticket/product backup only. Disabled by default on Render.', 'pos/hourly':'Optional POS hourly backup only. reporting.site sync is the primary hourly/daily source.', 'uber':'Uber daily workbook: LADonuts_Weekly_Input_TemplateV3.xlsx', 'friedas':'Frieda Square item exports: items-YYYY-MM-DD-YYYY-MM-DD.csv', 'production':'Production plan and baker/cook sheets XLSX' }[x] || '';
}
...
function renderSync() {
  ...
  return `<section class="card"><h2>Live Fallback</h2>${dateControls()}<p>0.2.31 uses reporting.site as the primary POS source. Use Live POS only for the selected date, or Sync hourly benchmarks to fetch WTD, same-day last week and 4-week comparison dates.</p><div class="actions"><button class="primary" data-action="sync-current-plus-last-days">Sync current + last days</button><button class="secondary" data-action="sync-all">Sync selected day only</button><button class="secondary" data-action="sync-uber">Live Uber only</button><button class="secondary" data-action="sync-square">Live Square API only</button><button class="ghost" data-action="diagnostics">Refresh diagnostics</button></div><div class="notice">Recommended env: <code>ENABLE_LIVE_FALLBACK=true</code>, <code>BENCHMARK_SYNC_WEEKS=4</code>, <code>ENABLE_LIVE_CONNECTORS=false</code>, <code>LIVE_FALLBACK_SOURCES=pos</code>. Uber/Frieda are imported from files unless explicitly synced. This avoids full-history scraping.</div></section><section class="card"><h2>Connector status</h2>${statusTable(s)}</section><section class="card"><h2>Environment status</h2>${envStatus()}</section><section class="card"><h2>Last sync runs</h2>${syncRuns()}</section>`;
}
...
async function syncAction(action) {
  ...
  const route = isCurrentPlusLast ? '/api/sync/pos/current-plus-last-days' : isSelectedDayOnly ? '/api/sync/pos/day-browser' : action === 'sync-uber' ? '/api/sync/uber' : '/api/sync/square';
  $('#syncStatus').textContent = isCurrentPlusLast ? `Syncing current + benchmark days for ${date}...` : isSelectedDayOnly ? `Syncing selected POS day for ${date}...` : 'Sync running...';
  setButtonsDisabled(true);
  try {
    const data = await postJson(route, { reportingDate: date, forceLiveFallback: isCurrentPlusLast, syncBenchmarks: isCurrentPlusLast }, false);
    state.live = data.live;
    await loadConfig();
    const syncedStores = Object.keys(data.result?.reportingPOS || data.result?.latestResult?.reportingPOS || {}).join(', ');
    const dates = data.result?.dates || [];
    $('#syncStatus').textContent = data.ok ? (isCurrentPlusLast ? `Synced ${dates.length || 'current + benchmark'} POS day(s).` : (syncedStores ? `POS synced: ${syncedStores}` : 'Sync updated')) : 'Sync not complete';
  } finally {
