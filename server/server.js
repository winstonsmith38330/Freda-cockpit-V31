...
function browserSyncEnvStatus() {
  return {
    ...
    note: '0.2.31 makes browser POS sync the primary UI path, forces the selected date, syncs stores sequentially with partial saves, and keeps runtime hourly extraction.'
  };
}
...
app.get('/api/diagnostics/browser-sync', asyncRoute(async (_req, res) => {
  res.json({ ok: true, version: '0.2.31', diagnostics: await browserSyncProbe() });
}));
...
app.post('/api/sync/pos/day', asyncRoute(async (req, res) => {
  // 0.2.31: when browser sync is enabled, the normal POS button uses the same
...
app.post('/api/captures/browser', (req, res) => {
  ...
  saveLive(next);
  res.json({ ok: true, parsed, live: liveMerged(), note: 'Manual capture is emergency/diagnostic only. Normal 0.2.31 workflow is file-import baseline plus isolated POS day/backfill fallback.' });
});
...
app.listen(PORT, () => console.log(`Freda Ops Cockpit Beta 0.2.31 running on http://localhost:${PORT}`));
...
async function runPosBrowserSequential(req, res) {
  ...
  for (const store of stores) {
    ...
    // 0.2.31: save immediately after each store, so a later store timeout cannot
...
async function runPosBenchmarks(req, res) {
  ...
  // 0.2.31: this is the Freda-facing "Sync current + last days" path.
